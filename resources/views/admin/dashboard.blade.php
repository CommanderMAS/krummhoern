            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        Sie haben bereits {{count($dataMessage)}} Mal Änderungen gespeichert. Zuletzt am {{date('d.m.Y',date_timestamp_get($dataMessage[0]['created_at']))}}<br/>
                        Letzte Daten: IBAN <span style="color:yellow"> {{Crypt::decrypt($dataMessage[0]['iban'])}}</span> Sonstiges <span style="color:yellow">{{Crypt::decrypt($dataMessage[0]['email'])}}</span>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @if (today() < '2023-08-01 00:00:00')
                            <form method="POST" action="{{ url('/dashboard') }}" enctype='multipart/form-data'>
                                ...
                                <div>
                                    ...
                                    <div>
                                        <input type="text" id="kontoinhaber" name="kontoinhaber" placeholder="Kontoinhaber">
                                    </div>
                                </div>
                                <br/>
                                ...
                                <div>
                                    ...
                                    <div>
                                        <input type="text" id="sonstiges" name="sonstiges" placeholder="Möchten sie uns noch etwas mitteilen?">
                                    </div>
                                </div>
                                @endif
                                <br/>
                                ...
                                <div class="col-md-12 row form-group float-right">
                                    <button type="submit" class="btn btn-primary btnbook">
